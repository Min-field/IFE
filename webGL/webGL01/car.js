...
function initCar(){
    ...
    (function addWheels(){
        ...
        for(var i = 0; i < 4; i++){
            wheels.push(new THREE.Mesh(new THREE.TorusGeometry(0.5, 0.25, 12, 18), material));
    ...
    (function addWindows(){
        var windowLeft = new THREE.Mesh(new THREE.CubeGeometry(2, 1, 0.005), new THREE.MeshLambertMaterial({
            color: 0x644266
        })); 
        windowLeft.position.set(2, 0, 1.5); 
        scene.add(windowLeft); 

        var windowRight = new THREE.Mesh(new THREE.CubeGeometry(2, 1, 0.005), new THREE.MeshLambertMaterial({
            color: 0x644266
        })); 
        windowRight.position.set(2, 0 , -1.5); 
